...
ALLOWED_CHATS = [-1002408347623, -1003761187223] # Добавлен префикс -100 (стандарт для супергрупп)
...
# --- БАЗА ДАННЫХ ---
conn = sqlite3.connect("abode_gods_v11.db", check_same_thread=False)
cursor = conn.cursor()
...
async def profile_handler(message: types.Message):
    ...
    await message.answer(
        f"👤 {target.full_name}\n"
        f"💠 Очки силы: {res[0]}\n"
        f"📈 Ранг: {get_rank(res[1])}\n"
        f"💬 Активность: {res[1]} сообщ."
    )

# --- 2. ЛОТЕРЕЯ СО СТАВКОЙ ---
...
async def lottery_handler(message: types.Message):
    ...
    r = random.random() * 100
    # x0 (40%), x1 (35%), x2 (15%), x5 (7%), x10 (2.5%), x50 (0.5%)
    if r < 0.5: mult = 50
    elif r < 3: mult = 10
    elif r < 10: mult = 5
    elif r < 25: mult = 2
...
async def pvp_start(message: types.Message):
    ...
    if len(bals) < 2 or any(b[0] < price for b in bals):
        return await message.reply(f"Для боя нужно {price} 💠 у обоих!")

    kb = InlineKeyboardMarkup().add(InlineKeyboardButton(f"Принять ({price} 💠)", callback_data=f"pvp_{user_id}_{target_id}_{price}"))
    await message.answer(f"🔫 @{message.from_user.username} вызывает @{message.reply_to_message.from_user.username}!\nСтавка: {price} 💠", reply_markup=kb)
...
async def pvp_callback(callback: types.CallbackQuery):
    ...
    if any(b[0] < price for b in cursor.fetchall()):
        return await callback.message.edit_text("❌ Бой отменен: не хватает очков.")
